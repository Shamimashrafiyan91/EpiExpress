...
build_multihead_attention_mlp <- function(input_dim, rate1, rate2, num_heads = 4) {
  ...
  dense2 <- dense1 %>%
    layer_dense(units = 50, activation = "relu", name = "dense2") %>%
    layer_dropout(rate = rate2, name = "dropout2")
...
for (p in file_contents) {
  ...
  tryCatch({
    ...
    cat("Best val_loss:", validation_err,
        "with dropout1:", b_hyper_dropouts, "dropout2:", b_hyper_dropouts2,
        "seed:", best_seed, "\n")
   ...
   # feature_importance <- colMeans(att_matrix)
    ...
    write.csv(att_tbl_avg, file.path(att_dir, paste0(gene_name, "_MULTIattention_avg.csv")), row.names = FALSE)
    ...
    for (h in 1:4) {
      ...
      write.csv(att_tbl_head, file.path(att_dir, paste0(gene_name, "_MULTIattention_head", h, ".csv")), row.names = FALSE)
    }
    
    
    
    ## IGNORE PLOTTING TOP FEATURES
    # ---------- Plot top-k features by attention ----------
