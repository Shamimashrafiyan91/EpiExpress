...
parallel_vars <- c("build_multihead_attention_mlp", "dropouts", "seeds", 
...
build_multihead_attention_mlp <- function(input_dim, rate1, num_heads = 4) {
  input_layer <- layer_input(shape = c(input_dim), name = "input")
  
  # Create multiple attention heads
  attention_heads <- lapply(1:num_heads, function(h) {
    input_layer %>%
      layer_dense(units = input_dim,
                  activation = "softmax",
                  name = paste0("attention_weights_head", h)) %>%
      { layer_multiply(list(input_layer, .), name = paste0("weighted_input_head", h)) }
  })
  
  # Concatenate the outputs of all heads
  multihead_output <- layer_concatenate(attention_heads, name = "multihead_concat")
  
  # Single hidden layer MLP (100 nodes + dropout)
  dense1 <- multihead_output %>%
...
for (p in file_contents) {
  ...
  tryCatch({
    ...
    best_loss2 <- Inf
    b_hyper_dropouts <- NA
    b_hyper_dropouts2 <- NA
    best_seed <- NA
    
    # ---------- Tuning: 2 hidden layers with attention ----------
    cat("Two hidden layers mode with Attention\n")
    for (j in 1:length(dropouts)) {
        
        # Export data/vars to workers
        ...
        a2 <- parLapply(cl, seeds, function(seed) {
          ...
          model <- build_multihead_attention_mlp(
            input_dim = input_dim,
            rate1 = dropouts[j]
          ...
          history <- model %>% fit(
            ...
            shuffle = FALSE,            # NOTE: keras arg is 'shuffle'
            validation_split = 0.3,
            verbose = 0,
    ...
    Best_model <- build_multihead_attention_mlp(
      input_dim = ncol(training),
    ...
    stat_dir <- "/projects/apog/work/models/1MB/attention_mode/multihead/NN1_multi/stat"
    dir.create(stat_dir, recursive = TRUE, showWarnings = FALSE)
    saveRDS(acc_nested_list, file.path(stat_dir, "NN1_MULTIattention_4mode.RDS"))
    
    # ---------- Extract & save attention weights ----------
    # Build a small model to output the attention layer
    att_extractor <- keras_model(
      inputs = Best_model$input,
      # outputs = get_layer(Best_model, "attention_weights")$output
      outputs = lapply(1:4, function(h) get_layer(Best_model, paste0("attention_weights_head", h))$output)
      
    )
    att_matrix <- predict(att_extractor, training, batch_size = 128)  # n_samples x n_features
    
    # Global feature importance = mean attention over samples
    # feature_importance <- colMeans(att_matrix)
    
    feature_importances <- lapply(att_matrix, colMeans)
    
    global_importance <- Reduce("+", feature_importances) / length(feature_importances)
    
    # ---- Save results ----
    att_dir <- "/projects/apog/work/models/1MB/attention_mode/multihead/NN1_multi/weights"
    dir.create(att_dir, recursive = TRUE, showWarnings = FALSE)
    
    # Save averaged importance
    att_tbl_avg <- data.frame(
      feature = colnames(training),
      mean_attention = global_importance
    )
    write.csv(att_tbl_avg, file.path(att_dir, paste0(gene_name, "_MULTI_NN1_attention_avg.csv")), row.names = FALSE)
    
    # Save per-head importance
    for (h in 1:4) {
      att_tbl_head <- data.frame(
        feature = colnames(training),
        mean_attention = feature_importances[[h]]
      )
      write.csv(att_tbl_head, file.path(att_dir, paste0(gene_name, "_MULTI_NN1_attention_head", h, ".csv")), row.names = FALSE)
    }
    ...
    # Stop cluster for this gene
    stopCluster(cl)
    
  }, error = function(e) {
    cat("ERROR in gene", p, ":", conditionMessage(e), "\n")
    # try to safely stop cluster if it exists
